...
export const CLINIC = {
  ...
  /**
   * ★★ 운영자가 정한 슬로건 (2026-08-21, "환자 중심 진료, 소통하는 치과
   *    이걸로 다시 바꿔") ★★ 원래는 '통증과 불편함을 고려하는 치과' 였다.
   * ⚠️ 이 한 줄이 여러 자리에 동시에 나간다 — 첫 화면 큰 제목, 모든 페이지의
   *    브라우저 제목·공유 카드(og:title), llms.txt, 그리고 schema.org 의
   *    slogan / 소개 영상 설명. 여기만 고치면 전부 같이 바뀐다(그래서 화면에
   *    문자열을 직접 적지 않는다).
   */
  tagline: '환자 중심 진료, 소통하는 치과',
  description:
